document.addEventListener('DOMContentLoaded', () => {
      ...
      if (tab.url?.includes('net20.cc')) {
        chrome.tabs.sendMessage(tab.id, { action: 'getStatus' }, (response) => {
          if (response?.allTracks) {
            const trackIndex = prompt(
              action === 'selectTrack1' ? 'Select Primary Track (0-based index):' : 'Select Secondary Track (0-based index):',
              '0'
            );
  ...
  function updateFromResponse(response) {
    ...
    statusEl.textContent = response.active ? '✅ Dual subtitles ACTIVE' : 
                          trackTotal ? `${trackTotal} tracks ready` : 
                          response.domInfo?.iframes ? `${response.domInfo.iframes} iframe(s)` : 
                          'Scanning...';
